...
def test_batch_shapes_and_uniqueness() -> None:
    ...
    ds = QueryUniqueBatchDataset(
        x, y, query_size, batch_size, n_batches, random_state=42
    )
...
def test_valueerror_mismatched_lengths() -> None:
    sample_size: Final[int] = 100
    x = torch.randn(sample_size, 10)
    y = torch.randint(low=0, high=2, size=(sample_size - 1,))
    with pytest.raises(ValueError, match="Dimension mismatch"):
        _ = QueryUniqueBatchDataset(
            x, y, query_size=5, batch_size=5, n_batches=1, random_state=0
        )
...
def test_valueerror_insufficient_data() -> None:
    ...
    with pytest.raises(ValueError, match="Not enough data"):
        _ = QueryUniqueBatchDataset(
            x, y, query_size=10, batch_size=10, n_batches=5, random_state=0
        )


def test_determinism() -> None:
    x, y = generate_synthetic_data(num_samples=500, num_features=10, num_classes=3)
